  ...
  var Plugin = class extends AppPlugin {
    ...
    onLoad() {
      ...
      this._statusBarItem = this.ui.addStatusBarItem({
        ...
        onClick: /* @__PURE__ */ __name(() => this._openManagerPanel(), "onClick")
      });
      this.ui.addCommandPaletteCommand({
        label: "Open Plugins Manager",
        icon: "box",
        onSelected: /* @__PURE__ */ __name(() => this._openManagerPanel(), "onSelected")
      });
      if (localStorage.getItem("pm_self_update_pending") === "true") {
        ...
        setTimeout(async () => {
          try {
            ...
            console.error("Failed to reopen panel after self-update", e);
          }
        }, 800);
      }
    ...
    async startAutomatedUpdateChecker() {
      ...
      const now = Date.now();
      if (!lastCheck || now - parseInt(lastCheck) > 24 * 60 * 60 * 1e3) {
        await this.checkForAllUpdatesInBackground();
        localStorage.setItem("pm_last_update_check", now.toString());
      }
      this._updateIntervalId = setInterval(() => this.checkForAllUpdatesInBackground(), 12 * 60 * 60 * 1e3);
    }
    async checkForAllUpdatesInBackground(options = {}) {
      const isManual = options.manual === true;
      if (isManual) {
        this.ui.addToaster({ title: "Checking for updates...", message: "This may take a moment depending on the number of plugins.", autoDestroyTime: 3e3, dismissible: true });
      }
      try {
        ...
        let checkCount = 0;
        const MAX_CHECKS = isManual ? 100 : 50;
        for (const p of allPlugins) {
          ...
          try {
            ...
            if (repo && this._isValidGithubUrl(repo)) {
              ...
              await new Promise((r) => setTimeout(r, isManual ? 500 : 1e3));
            }
          } catch (e) {
            if (e.message && (e.message.includes("rate limit") || e.message.includes("403"))) {
              ...
              if (isManual) {
                this.ui.addToaster({ title: "Rate Limit Hit", message: "GitHub API rate limit reached. Please try again later or add a PAT in Settings.", autoDestroyTime: 5e3, dismissible: true });
              }
              localStorage.setItem("pm_last_update_check", (Date.now() + 6 * 60 * 60 * 1e3).toString());
              break;
            }
          }
        }
        ...
        if (updateCount > 0) {
          ...
          let previousUpdates = {};
          try {
            previousUpdates = JSON.parse(previousUpdatesStr);
          } catch (e) {
          }
        ...
        if (isManual) {
          this.ui.addToaster({ title: "Update Check Failed", message: e.message, autoDestroyTime: 5e3, dismissible: true });
        }
      }
    }
    ...
    _readUpdateCache() {
      let raw;
      try {
        raw = JSON.parse(localStorage.getItem("pm_updates_available") || "{}");
      } catch (e) {
        return {};
      }
      const out = {};
    ...
    _writeUpdateCache(cache) {
      try {
        localStorage.setItem("pm_updates_available", JSON.stringify(cache));
      } catch (e) {
      }
    }
    /**
    ...
    _isRemoteVersionNewer(remote, installed) {
      ...
      const nums = /* @__PURE__ */ __name((v) => (String(v).match(/\d+/g) || []).map(Number), "nums");
    ...
    _installInstantTooltips(root) {
      ...
      const show = /* @__PURE__ */ __name((target, ev) => {
      ...
      const onOver = /* @__PURE__ */ __name((e) => {
      ...
      const onScrollOrBlur = /* @__PURE__ */ __name(() => hide(), "onScrollOrBlur");
    ...
    bindEvents(container, panel) {
      ...
      container.querySelector("#pm-save-settings").addEventListener("click", async () => {
        ...
        this.ui.addToaster({ title: "Settings Saved", dismissible: true, autoDestroyTime: 3e3 });
      });
      container.querySelector("#pm-restore-github-btn").addEventListener("click", async (ev) => {
        ...
        try {
          ...
          this.ui.addToaster({ title: "GitHub Restore Failed", message: e.message, autoDestroyTime: 6e3, dismissible: true });
        } finally {
          btn.disabled = false;
        }
      });
    ...
    async _saveManagerSettings(overrides = {}) {
      ...
      if (overrides.communityRepos !== void 0) {
      ...
      if (overrides.savedThemes !== void 0) {
    ...
    async _renderWorkspaceSummary(container) {
      ...
      try {
        ...
        const autoBackupState = this._ghBackupRepo ? `Auto-backup \u2192 GitHub (${this._ghBackupRepo})` : "Auto-backup inactive \u2014 set a repository under GitHub Backup below";
        summaryEl.innerHTML = `
    ...
    async _filterDiscoverList(container) {
      ...
      if (searchTerm) {
        filtered = filtered.filter((item) => {
          return item.name.toLowerCase().includes(searchTerm) || item.description.toLowerCase().includes(searchTerm) || item.category.toLowerCase().includes(searchTerm);
        });
      }
        ...
        if (isIncompatible) {
          ...
          recheckBtn.addEventListener("click", async () => {
            ...
            try {
              ...
              localStorage.setItem("pm_incompatible", JSON.stringify(this._incompatiblePlugins));
              this.ui.addToaster({ title: "Compatible!", message: `${item.name} is now available to install.`, autoDestroyTime: 3e3, dismissible: true });
              this._renderDiscoverCards(container, items);
            } catch (e) {
              this.ui.addToaster({ title: "Still Incompatible", message: e.message, autoDestroyTime: 4e3, dismissible: true });
              recheckBtn.innerText = "Recheck";
            ...
            try {
              if (isTheme) {
                ...
                if (existingIdx > -1) {
                  ...
                  this._savedThemes[existingIdx].date = (/* @__PURE__ */ new Date()).toISOString();
                } else {
                ...
                this._saveThemes();
                this._autoExport();
                this.ui.addToaster({ title: "Theme Saved!", message: `Saved "${item.name}" to your Theme Library.`, autoDestroyTime: 4e3, dismissible: true });
                installBtn.className = "pm-btn";
                ...
                this.ui.addToaster({ title: `Successfully installed ${json.name}`, autoDestroyTime: 3e3, dismissible: true });
                installBtn.innerText = "Installed";
                this.loadPlugins(container);
              }
            } catch (err) {
              this._incompatiblePlugins[item.url] = { name: item.name, error: err.message, date: (/* @__PURE__ */ new Date()).toISOString() };
              localStorage.setItem("pm_incompatible", JSON.stringify(this._incompatiblePlugins));
              this.ui.addToaster({ title: "Failed", message: err.message, autoDestroyTime: 5e3, dismissible: true });
              this._filterDiscoverList(container);
            }
          });
        }
...
This removes it from the official Plugins panel. You can re-enable it later from Plugins Manager.`)) {
      ...
      this._disabledPlugins[sourceRepo] = {
        ...
        dateDisabled: (/* @__PURE__ */ new Date()).toISOString()
      };
    ...
    async _enableDisabledPlugin(disabledPlugin, panelContainer, enableBtn) {
      ...
      try {
        ...
        this.ui.addToaster({ title: "Enable Failed", message: e.message, dismissible: true, autoDestroyTime: 5e3 });
      }
    }
...
This will overwrite local code with the latest from GitHub, even if the version number hasn't changed.`)) return;
            try {
              ...
              if (isSelfUpdate) {
                ...
                this._autoExport();
                this.ui.addToaster({ title: "Reinstalled", message: `${conf.name} has been reinstalled from source.`, autoDestroyTime: 3e3, dismissible: true });
              }
              ...
              this.ui.addToaster({ title: "Reinstall Failed", message: e.message, autoDestroyTime: 5e3, dismissible: true });
              reinstallBtn.innerHTML = "";
        ...
        linkBtn.addEventListener("click", async () => {
          ...
          if (repoUrl === "") {
            ...
            this.ui.addToaster({ title: "Invalid URL", message: "Please enter a valid github.com URL.", autoDestroyTime: 4e3, dismissible: true });
            return;
          }
          try {
            ...
            this.ui.addToaster({ title: "Repo Updated", message: repoUrl ? `${conf.name} linked to ${repoUrl}.` : `${conf.name} link removed.`, autoDestroyTime: 4e3, dismissible: true });
            this.loadPlugins(panelContainer);
          } catch (e) {
            this.ui.addToaster({ title: "Update Failed", message: e.message, autoDestroyTime: 5e3, dismissible: true });
          }
        });
        const deleteBtn = document.createElement("button");
        deleteBtn.className = "pm-btn danger pm-btn-delete";
        deleteBtn.title = "Delete Plugin";
        deleteBtn.appendChild(this.ui.createIcon("trash"));
        actionsContainer.appendChild(deleteBtn);
        deleteBtn.addEventListener("click", async () => {
          if (confirm(`Are you sure you want to delete ${conf.name}?`)) {
            await p.trashPlugin();
            this._autoExport();
            this.ui.addToaster({ title: "Plugin deleted", dismissible: true, autoDestroyTime: 3e3 });
            this.loadPlugins(panelContainer);
          }
        });
    ...
    _renderThemesList(container) {
      ...
      this._savedThemes.forEach((theme, idx) => {
        ...
        card.innerHTML = `
                <div class="pm-card-info">
                    ...
                    <p class="pm-meta-text">
                        ${theme.css.length} chars \xB7 Added ${new Date(theme.date).toLocaleDateString()}
                        ${theme.source ? ` \xB7 <span class="pm-card-url" data-external-url="${this._escHtml(theme.source)}">${this._escHtml(theme.source)}</span>` : ""}
                    </p>
                </div>
                <div class="pm-card-actions"></div>
            `;
        ...
        copyBtn.addEventListener("click", async () => {
          ...
          this.ui.addToaster({ title: "Copied!", message: `${theme.name} CSS copied to clipboard.`, autoDestroyTime: 3e3, dismissible: true });
        });
    ...
    async _addThemeFromGithub(container) {
      ...
      if (!url) return;
      this.ui.addToaster({ title: "Fetching theme CSS...", autoDestroyTime: 2e3, dismissible: true });
      try {
        ...
        this._renderThemesList(container);
        this._renderWorkspaceSummary(container);
        this.ui.addToaster({ title: "Theme Saved", message: `"${name}" added to your Theme Library.`, autoDestroyTime: 3e3, dismissible: true });
      } catch (fetchErr) {
    ...
    _showManualThemePasteDialog(container, sourceUrl, errorMsg) {
      ...
      tempDiv.querySelector("#pm-manual-theme-save").addEventListener("click", () => {
        ...
        if (!name || !css) {
          this.ui.addToaster({ title: "Missing Fields", message: "Please provide both a name and CSS.", autoDestroyTime: 3e3, dismissible: true });
          return;
        }
        this._savedThemes.push({
          id: Date.now().toString(36),
    ...
    _showEditThemeDialog(container, idx) {
      ...
      tempDiv.querySelector("#pm-edit-theme-save").addEventListener("click", () => {
        ...
        this._renderThemesList(container);
        this._renderWorkspaceSummary(container);
        this.ui.addToaster({ title: "Theme Updated", message: `"${name}" has been updated.`, autoDestroyTime: 3e3, dismissible: true });
      });
    }
    ...
    _showPromptModal(title, message, defaultValue = "") {
      return new Promise((resolve) => {
        ...
        const close = /* @__PURE__ */ __name((value) => {
          this._closeModal(tempDiv);
          resolve(value);
        }, "close");
    ...
    async _exportAllThemes() {
      if (this._savedThemes.length === 0) {
        this.ui.addToaster({ title: "No Themes", message: "No themes saved to export.", autoDestroyTime: 3e3, dismissible: true });
        return;
      }
...
${t.css}`).join("\n\n");
      ...
      tempDiv.querySelector("#pm-themes-export-copy").addEventListener("click", async (e) => {
        ...
        setTimeout(() => e.target.innerText = orig, 2e3);
      });
    ...
    async previewTheme(repoUrl) {
      try {
        ...
        if (images.length === 0) {
          this.ui.addToaster({ title: "No Previews", message: "No images found in the theme's README.", autoDestroyTime: 3e3, dismissible: true });
          return;
        }
        const overlayHtml = `
        ...
        this.ui.addToaster({ title: "Preview Failed", message: "Could not load theme preview images.", autoDestroyTime: 3e3, dismissible: true });
      }
    }
    /**
    ...
    async _fetchThemeCSS(repoUrl) {
      ...
      try {
        ...
        const candidates = files.filter(
          (f) => f.type === "file" && !f.name.includes(".") && f.name.toLowerCase().includes("css")
        );
    ...
    _getBackupConfigSnapshot(baseJson, liveConfig) {
      ...
      for (const key of reservedKeys) {
        if (baseSnapshot[key] !== void 0 && liveSnapshot[key] === void 0) {
          liveSnapshot[key] = baseSnapshot[key];
        }
      }
      if (baseSnapshot.__source_repo !== void 0) liveSnapshot.__source_repo = baseSnapshot.__source_repo;
      if (baseSnapshot.__source_files !== void 0) liveSnapshot.__source_files = baseSnapshot.__source_files;
      return liveSnapshot;
    }
    ...
    _normalizeImportedBackup(parsed, typeFilter) {
      ...
      const items = Array.isArray(parsed.items) ? parsed.items : Array.isArray(parsed.plugins) ? parsed.plugins : null;
      if (!items) {
    ...
    async _restoreImportedManagerSettings(managerSettings, container) {
      ...
      await this._saveManagerSettings({
        communityRepos: typeof managerSettings.communityRepos === "string" ? managerSettings.communityRepos : void 0,
        savedThemes: Array.isArray(managerSettings.savedThemes) ? managerSettings.savedThemes : void 0,
        ghBackupRepo: typeof managerSettings.ghBackupRepo === "string" ? managerSettings.ghBackupRepo : void 0,
        ghBackupBranch: typeof managerSettings.ghBackupBranch === "string" ? managerSettings.ghBackupBranch : void 0,
        ghBackupPath: typeof managerSettings.ghBackupPath === "string" ? managerSettings.ghBackupPath : void 0
      });
      if (container) {
    ...
    _topoSortCollections(items, colGuidToName) {
      ...
      const deps = {};
      for (const item of items) {
        deps[item.name] = /* @__PURE__ */ new Set();
    ...
    async _autoExport() {
      ...
      this._autoExportTimer = setTimeout(() => {
        this._autoExportTimer = null;
        this._runAutoExport();
      }, 400);
    }
    async _runAutoExport() {
      try {
        ...
        this.ui.addToaster({ title: "Auto-Backup Failed", message: e.message, autoDestroyTime: 6e3, dismissible: true });
      }
    }
    ...
    async _showGithubRestorePicker(container) {
      ...
      let page = 1;
      let total = 0;
      const appendRows = /* @__PURE__ */ __name((commits) => {
        for (const c of commits) {
          ...
          rowBtn.innerHTML = `<strong>${this._escHtml(label)}</strong>${total === 0 ? " \u2014 latest" : ""}
                    <span style="opacity:.7;font-size:12px;"> \xB7 ${this._escHtml(c.sha.slice(0, 7))}</span>`;
      ...
      const maybeHideMore = /* @__PURE__ */ __name((batch) => {
    ...
    async _pushBackupToGithub(jsonStr) {
      ...
      for (let attempt = 0; ; attempt++) {
        ...
        const body = {
          message: `thymer backup ${(/* @__PURE__ */ new Date()).toISOString()} (${this._getWorkspaceName()})`,
    ...
    _sanitizePluginConfig(jsonConf, { allowCustom = false, preserveUnknownKeys = false } = {}) {
      if (preserveUnknownKeys) {
        const sanitized2 = this._cloneJsonValue(jsonConf || {});
        if (!allowCustom) delete sanitized2.custom;
        if (allowCustom && sanitized2.custom !== void 0) {
          const customJson = JSON.stringify(sanitized2.custom);
      ...
      if (allowCustom && jsonConf.custom !== void 0) {
    ...
    _looksLikeCSS(text) {
      ...
      const sample = text.substring(0, 2e3);
    ...
    async _fetchGithubFile(owner, repo, branch, path) {
      if (this.githubPat) {
        ...
        if (!res2.ok) throw new Error(`Failed to fetch ${path} (${res2.status})`);
    ...
    async fetchGithubRepo(url, { sourceFiles } = {}) {
      ...
      const buildSourceFiles = /* @__PURE__ */ __name((branch2, jsonName, jsName, cssName) => {
        const sf = { branch: branch2, json: jsonName, js: jsName };
        if (cssName) sf.css = cssName;
        return sf;
      }, "buildSourceFiles");
      if (sourceFiles && sourceFiles.branch && sourceFiles.json && sourceFiles.js) {
        try {
          const pluginJson2 = JSON.parse(await this._fetchGithubFile(owner, repo, sourceFiles.branch, `${prefix}${sourceFiles.json}`));
          const fetchedJs2 = await this._fetchGithubFile(owner, repo, sourceFiles.branch, `${prefix}${sourceFiles.js}`);
          const { js: pluginJs2, jsName: resolvedJsName2 } = await this._preferDistBundle(owner, repo, sourceFiles.branch, prefix, sourceFiles.js, fetchedJs2);
          pluginJson2.__source_repo = url;
          pluginJson2.__source_files = buildSourceFiles(sourceFiles.branch, sourceFiles.json, resolvedJsName2, sourceFiles.css);
          const result2 = { json: pluginJson2, js: pluginJs2 };
          if (sourceFiles.css) {
            try {
              result2.css = await this._fetchGithubFile(owner, repo, sourceFiles.branch, `${prefix}${sourceFiles.css}`);
            } catch (e) {
            }
          }
          return result2;
      ...
      for (const branch2 of ["main", "master"]) {
        let foundJson = null;
        let foundJsonName = null;
        for (const jsonName of jsonCandidates) {
          try {
            const text = await this._fetchGithubFile(owner, repo, branch2, `${prefix}${jsonName}`);
            try {
        ...
        for (const jsName of jsCandidates) {
          try {
            const fetchedJs2 = await this._fetchGithubFile(owner, repo, branch2, `${prefix}${jsName}`);
            const { js: pluginJs2, jsName: resolvedJsName2 } = await this._preferDistBundle(owner, repo, branch2, prefix, jsName, fetchedJs2);
            foundJson.__source_repo = url;
            const cssCandidates = ["plugin.css", "styles.css", `${repo}-plugin.css`, "Custom CSS"];
            const result2 = { json: foundJson, js: pluginJs2 };
            let foundCssName = null;
            for (const cssName of cssCandidates) {
              try {
                result2.css = await this._fetchGithubFile(owner, repo, branch2, `${prefix}${cssName}`);
                foundCssName = cssName;
                break;
              } catch (e) {
              }
            }
            foundJson.__source_files = buildSourceFiles(branch2, foundJsonName, resolvedJsName2, foundCssName);
            return result2;
          } catch (e) {
          }
        }
    ...
    async showInstallDialog(container, typeFilter) {
      ...
      if (!url) return;
      try {
        this.ui.addToaster({ title: "Fetching plugin...", autoDestroyTime: 2e3, dismissible: true });
        ...
        this.ui.addToaster({ title: "Install Failed", message: err.message, autoDestroyTime: 5e3, dismissible: true });
      }
    }
...
Click OK for Plugin, or Cancel for Collection Plugin.`);
      ...
      if (existingConf && existingConf.custom !== void 0 && (!trustedConfig || jsonConf.custom === void 0)) {
      ...
      try {
        ...
        if (pTypeNorm === "collection" && Array.isArray(sanitizedConf.fields) && sanitizedConf.fields.length > 0) {
          ...
          if (hasColNames) {
            const allCollections2 = await this.data.getAllCollections();
            const nameToGuid = {};
            for (const tc of allCollections2) {
              try {
            ...
            sanitizedConf.fields = sanitizedConf.fields.map((f) => {
              if (f.filter_colguid && nameToGuid[f.filter_colname]) {
                const { filter_colname: filter_colname2, ...rest2 } = f;
                return { ...rest2, filter_colguid: nameToGuid[f.filter_colname] };
              }
        ...
        if (createdContainer) {
          try {
            await targetPlugin.trashPlugin();
          } catch (_) {
          }
        }
        throw e;
...
Warning: This will overwrite any local code modifications for these plugins.`)) {
      ...
      this.ui.addToaster({
        ...
        message: parts.join(". "),
        dismissible: true,
        autoDestroyTime: failedNames.length > 0 ? 8e3 : 5e3
      });
    }
      ...
      document.getElementById("pm-import-confirm").addEventListener("click", async () => {
        ...
        if (val.startsWith("[") || val.startsWith("{")) {
          try {
            ...
            this.ui.addToaster({ title: "Import Failed", message: e.message || "Invalid JSON format", autoDestroyTime: 5e3, dismissible: true });
            this._closeModal(tempDiv);
            return;
          }
        } else {
    ...
    async checkAndUpdatePlugin(pluginObj, currentConf, sourceRepo, btnEl, container, knownUpdateVersion = null, options = {}) {
      ...
      try {
        ...
        if (!forceUpdate && !this._isRemoteVersionNewer(remoteJson.version, currentConf.version)) {
          ...
          this.ui.addToaster({
            title: "Up to date",
            message: localAhead ? `${currentConf.name} (v${currentConf.version}) is ahead of GitHub (v${remoteJson.version || "none"}). Push the repo to sync. Use Reinstall to overwrite local anyway.` : `${currentConf.name} is already on the latest version.`,
            autoDestroyTime: localAhead ? 6e3 : 3e3,
            dismissible: true
          });
          ...
          const badge2 = document.getElementById(`vbadge-${pluginObj.getGuid()}`);
          if (badge2) {
            badge2.innerText = `v${currentConf.version}`;
            badge2.classList.remove("update");
          }
          setTimeout(() => {
            btnEl.innerHTML = "";
        ...
        const applyUpdate = /* @__PURE__ */ __name(async () => {
...
Warning: This will overwrite any local code modifications.`)) {
            ...
            if (isSelfUpdate) {
              ...
              this._autoExport();
              this.ui.addToaster({ title: "Update Successful", message: `${currentConf.name} updated to v${remoteJson.version}`, autoDestroyTime: 3e3, dismissible: true });
            }
        ...
        console.error(err);
        this.ui.addToaster({ title: "Update Failed", message: err.message, autoDestroyTime: 5e3, dismissible: true });
        btnEl.innerHTML = "";
        btnEl.appendChild(this.ui.createIcon("refresh"));
        btnEl.title = "Check Update";
        btnEl.disabled = false;
